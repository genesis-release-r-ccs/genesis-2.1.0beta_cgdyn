...
contains
  ...
  subroutine compute_dynvars(enefunc, dynamics, boundary, ensemble, domain, &
    ...
    if (dynamics%integrator == IntegratorLEAP) then
      ...
      do i = 1, natom
        vel_full(i,1) = 0.5_wip *(vel(i,1)+vel_ref(i,1))
        vel_full(i,2) = 0.5_wip *(vel(i,2)+vel_ref(i,2))
        vel_full(i,3) = 0.5_wip *(vel(i,3)+vel_ref(i,3))
      end do
      !$omp end parallel do
    else
      do i = 1, natom
        vel_full(i,1) = vel_ref(i,1)
        vel_full(i,2) = vel_ref(i,2)
        vel_full(i,3) = vel_ref(i,3)
      end do
    end if
    ...
    key = 0.0_dp
    kez = 0.0_dp
    !$omp parallel do private(i) reduction(+:kex,key,kez)
    do i = 1, natom
      kex = kex + mass(i)*vel_full(i,1)*vel_full(i,1)
      key = key + mass(i)*vel_full(i,2)*vel_full(i,2)
      kez = kez + mass(i)*vel_full(i,3)*vel_full(i,3)
    end do
    !$omp end parallel do

    ekin_sum  = 0.5_dp * (kex+key+kez)
    if (dynamics%integrator == IntegratorLEAP) ekin = ekin_sum
    ...
    if (dynamics%integrator == IntegratorLEAP) then
      ...
      do i = 1, natom
        vel_oldx = 2.0_wip*vel_full(i,1)-vel(i,1)
        vel_oldy = 2.0_wip*vel_full(i,2)-vel(i,2)
        vel_oldz = 2.0_wip*vel_full(i,3)-vel(i,3)
        ekin_old = ekin_old   &
                 + (vel_oldx*vel_oldx+vel_oldy*vel_oldy+vel_oldz*vel_oldz)&
                  *mass(i)
        ekin_new = ekin_new   &
                 + (vel(i,1)*vel(i,1) &
                   +vel(i,2)*vel(i,2) &
                   +vel(i,3)*vel(i,3))*mass(i)
        rmsg     = rmsg       &
                 + force(i,1)*force(i,1) &
                 + force(i,2)*force(i,2) &
                 + force(i,3)*force(i,3)
    ...
    if (ensemble%tpcontrol == TpcontrolLangevin .and. &
        dynamics%integrator == IntegratorLEAP) then
      scale_v = 1.0_wip + 0.5_wip*gamma_t*dt
      kex  = kex * scale_v
      key  = key * scale_v
      kez  = kez * scale_v
      ekin = ekin * scale_v
    ...
    call reduce_property(dynvars%energy%bond,               &
                         ...
                         kex, key, kez, ekin_old, ekin_new, ekin_sum, rmsg)
#endif
  ...
  subroutine reduce_property(val1 , val2 , val3 , val4 , val5 , val6 , val7 , &
    ...
    real(dp),                intent(inout) :: val22, val23, val24
    real(dp),                intent(inout) :: val25, val26, val27
    real(dp),                intent(inout) :: val28, val29, val30
    real(dp),                intent(inout) :: val31, val32, val33

#ifdef HAVE_MPI_GENESIS
    ! local variables
    real(dp)                 :: before_reduce(33), after_reduce(33)
    ...
    before_reduce(26) = val26
    before_reduce(27) = val27
    before_reduce(28) = val28
    before_reduce(29) = val29
    before_reduce(30) = val30
    before_reduce(31) = val31
    before_reduce(32) = val32
    before_reduce(33) = val33

    call mpi_allreduce(before_reduce, after_reduce, 33, mpi_real8, &
    ...
    val26 = after_reduce(26)
    val27 = after_reduce(27)
    val28 = after_reduce(28)
    val29 = after_reduce(29)
    val30 = after_reduce(30)
    val31 = after_reduce(31)
    val32 = after_reduce(32)
    val33 = after_reduce(33)
