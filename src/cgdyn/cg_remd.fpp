...
contains
  ...
  subroutine read_ctrl_remd(handle, rep_info)
    ...
    integer                  :: i, trep
    character(30)            :: cdim
    character(45)            :: numtmp, partmp, vartmp
    character(45)            :: fnctmp, cyctmp, seltmp
    character(45)            :: typtmp, adjtmp
  ...
  subroutine setup_solute_tempering(rep_info, restraints)
  ...
  subroutine setup_remd(rep_info, rst, dynamics, molecule, domain, &
  ...
  subroutine setup_remd_solute_tempering( rep_info, molecule, domain, &
    ...
    do i = 1, remd%dimension
      if (remd%types(i) == RemdSoluteTempering) then
        ...
        remd%solute_tempering(i)%is_solute(1:molecule%num_atoms) = 0
  ...
  subroutine setup_remd_solute_tempering_bonds( soltemp, domain, enefunc )
    ...
    integer            :: alist(num_unit), i, j, num

    soltemp%num_bonds = 0

    if ( .not. soltemp%sw_bonds ) return

    do i = 1, enefunc%num_bond_domain
      alist(1:num_unit) = enefunc%bond_list(1:2,i)
      num = 0
      do j = 1, num_unit
...
!     do ix = 1, enefunc%num_angl(i)
!       alist(1:num_unit) = enefunc%angl_list(1:num_unit,ix,i)
...
!     end do
  ...
  subroutine setup_remd_solute_tempering_dihedrals( soltemp, domain, enefunc )
    ...
    do i = 1, enefunc%num_dihe_domain
      ...
      alist(1:num_unit) = enefunc%dihe_list(1:num_unit,i)
  ...
  subroutine setup_remd_solute_tempering_contacts( soltemp, domain, enefunc )
    ...
    do i = 1, enefunc%num_contact_domain
      alist(1:num_unit) = enefunc%contact_list(1:num_unit,i)
  ...
  subroutine setup_remd_solute_tempering_lj( soltemp, domain, enefunc )
    ...
    ! local variables
    integer :: i, localcount, oldcount, org, ncell
    integer :: newcount
#ifdef HAVE_MPI_GENESIS
    integer :: ierror
#endif
    ...
    do i = 1, domain%num_atom_domain+domain%num_atom_boundary
      if ( soltemp%is_solute(domain%id_l2g(i)) > 0 ) then
        check_cls(domain%atom_cls_no(i)) = .true.
      end if
    end do
    ...
    do i = 1, domain%num_atom_domain
      if ( soltemp%is_solute(domain%id_l2g(i)) > 0 ) then
        org = lc_atom_cls(domain%atom_cls_no(i))
        domain%atom_cls_no(i) = org + enefunc%num_atom_cls
      end if
    end do
  ...
  subroutine run_remd(grotop, molecule, output, domain, enefunc, dynvars, &
  ...
  subroutine perform_replica_exchange(icycle, domain, enefunc, dynvars, &
    ...
    do i = 1, total_nreplicas-1
      do j = i+1, total_nreplicas
        remd%random_table(i,j) = real(random_get_legacy(iseed),wp)
        remd%random_table(j,i) = real(remd%random_table(i,j),wp)
      end do
    end do
    ...
    case(RemdRestraint)
      ...
      call restraint_remd(dimno, exptrn, domain, ensemble, &
                          enefunc, remd)

    case(RemdSoluteTempering)

      call solute_tempering_remd(dimno, exptrn, domain, ensemble,   &
                                 enefunc, remd, pairlist, boundary, &
                                 dynvars, output)
      ...
      if (counter_tuning(dimno) >= 2 * remd%autoadj(dimno)%trial_freq) then
        ...
        if ( remd%types(dimno) == RemdTemperature ) then
          ...
          do j = 1, domain%num_atom_domain
            domain%velocity(j,1) = domain%velocity(j,1)*factor
            domain%velocity(j,2) = domain%velocity(j,2)*factor
            domain%velocity(j,3) = domain%velocity(j,3)*factor
          end do
  ...
  subroutine remd_autoadj_displacement(dimno, remd)
    ...
    integer  :: parmsetid, umbrid, numbs
    real(wp) :: prob, diff, s, s2
  ...
  subroutine temperature_remd(dimno, exptrn, domain, dynvars, remd)
    ...
    integer                   :: parmsetid_i, parmsetid_j
    ...
    !
    eneval = real(dynvars%energy%total,wp)
  ...
  subroutine pressure_remd(dimno, exptrn, ensemble, boundary, remd)
    ...
    ! allgather volume
    !
    before_gather =  real(boundary%box_size_x &
                   * boundary%box_size_y &
                   * boundary%box_size_z,wp) 
#ifdef HAVE_MPI_GENESIS
    call mpi_allgather(before_gather, 1, mpi_wp_real, &
                       after_gather,  1, mpi_wp_real, &
                       mpi_comm_airplane, ierror)
  ...
  subroutine surface_tension_remd(dimno, exptrn, ensemble, boundary, remd)
    ...
    ! allgather area
    !
    before_gather =  real(boundary%box_size_x,wp) &
  ...
  subroutine restraint_remd(dimno, exptrn, domain, ensemble, enefunc, remd)
    ...
    call compute_energy_restraints_remd(.true., enefunc, domain, eneval)
    before_gather = eneval
    ...
    if (umbr_exist) then
      ...
      !
      call compute_energy_restraints_remd(.false., enefunc, domain, eneval)
  ...
  subroutine solute_tempering_remd(dimno, exptrn, domain, ensemble,  &
                                   enefunc, remd, pairlist,          &
                                   boundary, dynvars, output)
...
!   nc = size(domain%force(3,MaxAtom,:))
    nc = size(domain%force(:,3))
    nv = size(domain%virial_cellpair(3,:))
    if ( .not. allocated(force) ) then
      allocate(force(nc,3), &
               force_long(nc,3), &
               force_omp(nc,3,nthread), &
               force_pbc(nc,3,nthread), &
               virial_cell(3,nv))
    end if
    ...
    call compute_energy(pairlist, boundary,                     &
                        ensemble%use_barostat, .true., .true.,  &
                        enefunc, domain, dynvars)

    ! allgather potential energy
    !
    eneval = real(energy%total,wp)
    call mpi_bcast(eneval, 1, mpi_wp_real, 0, mpi_comm_country, ierror)
    ...
    do i = 1, total_nreplicas
      ...
      if (do_trial) then
        ...
        ! compute delta
        !
        beta     = 1.0_wp / ( KBOLTZ * real(ensemble%temperature,wp) )
        energy_i = remd%potential_energy(repid_i)
        energy_j = remd%potential_energy(repid_j)
  ...
  subroutine assign_condition_solute_tempering( soltemp, rest_param, &
    ...
    integer  :: i, n, ncell
    real(wp) :: coeff_full, coeff_half, tmp1, tmp2

    coeff_full = real(ensemble%temperature,wp) / rest_param
    ...
    if ( soltemp%sw_charge ) then
      ...
      do i = 1, domain%num_atom_domain
        if ( soltemp%is_solute(domain%id_l2g(i)) > 0 ) then
          domain%charge(i) = coeff_half * domain%charge(i)
        end if
      end do
...
!     end if
      ...
      ! angle and urey
!     do ix = 1, enefunc%num_angl(i)
!       alist(1:3) = enefunc%angl_list(1:3,ix,i)
!       num = 0
!       do j = 1, 3
!         if (soltemp%is_solute(alist(j)) > 0) num = num + 1
!       end do
...
!         wgt = coeff_full ** wgt
!         enefunc%angl_force_const(ix,i) = &
!                    wgt * enefunc%angl_force_const(ix,i)
!       end if
!       if ( enefunc%urey_force_const(ix,i) > EPS .and. &
!            soltemp%sw_ureys ) then
!         num = 0
...
!     end if
  ...
  subroutine get_replica_temperature(parmsetid, remd, ensemble, temperature)
    ...
    temperature = real(ensemble%temperature,wp)
  ...
  subroutine compute_energy_restraints_remd(get_coord, enefunc, domain, &
    ...
    eposi_omp(1:nthread) = 0.0_wp

    call compute_energy_restraints(get_coord, .false., domain,            &
                                   enefunc, domain%coord, force_omp,      &
                                   virial_omp, virial_ext_omp, eposi_omp, &
                                   ermsd, rmsd, ebonds, eemfit)

    eneval = real(ebonds + ermsd + eemfit,wp)

    return

  end subroutine compute_energy_restraints_remd

end module cg_remd_mod
