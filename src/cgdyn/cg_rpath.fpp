...
contains
  ...
  subroutine setup_rpath(rpath_info,rst, dynamics, molecule, domain, &
                         enefunc, rpath)
    ...
    if (dynamics%integrator == IntegratorPMTS ) &
        call error_msg('Setup_Rpath> IntegratorPMTS is not allowed')
    ...
    if (ikind /= RestraintsFuncPOSI .and. &
      ...
      call error_msg('Setup_Rpath> [ERROR] This CV is not allowed in cgdyn')
    endif
    ...
    enefunc%stats_delta(:)         = 0.0_dp
    enefunc%stats_grad(:,:,:)      = 0.0_dp
    enefunc%stats_force(:)         = 0.0_dp
    enefunc%stats_metric(:,:)      = 0.0_dp
    enefunc%stats_atom(:,:)        = 0
    enefunc%stats_mass(:,:)        = 0.0_dp
  ...
  subroutine assign_condition(rpath, enefunc, domain)
    ...
    ! local variables
    integer                  :: i, j, k, ncell_local, ifunc
    integer                  :: replicaid
    integer                  :: iatm, iatm_xyz
    ...
    case(RestraintsFuncPOSI)
      ...
      do i = 1, enefunc%num_restraint_domain

        j = enefunc%restraint_atom(i)
        k = enefunc%stats_id_atm2cv(j)

        enefunc%restraint_coord(1,i) = rpath%rest_reference(1,3*k-2,replicaid)
        enefunc%restraint_coord(2,i) = rpath%rest_reference(1,3*k-1,replicaid)
        enefunc%restraint_coord(3,i) = rpath%rest_reference(1,3*k  ,replicaid)
  ...
  subroutine run_rpath(grotop, molecule, output, domain, enefunc, dynvars, &
  ...
  subroutine reparametrize(rpath)
    ...
    smooth_dp = real(rpath%smooth,dp)
    ...
    if (main_rank) then
      ...
      do repid_i = 2, rpath%nreplica - 1
        do dimno = 1, rpath%dimension
          path_smooth(repid_i,dimno) = (1.0_dp-smooth_dp) * path(repid_i,dimno) + &
            (smooth_dp*0.5_dp)*(path(repid_i-1,dimno) + path(repid_i+1,dimno))
        end do
      end do
      ...
      do repid_i = 2, rpath%nreplica
        ...
        path_equi(repid_i) = (repid_i-1)* &
                path_leng(rpath%nreplica)/real(nrep_int,dp)
      end do
